-- Schema iniziale di PyClasse, applicato dalla Supabase CLI come migrazione.
...
create policy "students read class assignments" on public.class_assignments for select to authenticated using (
  ...
  and exists (select 1 from public.class_members cm where cm.class_id = class_id and cm.student_id = auth.uid())
);
...
create policy "students create own drafts" on public.submissions for insert to authenticated
with check (
  student_id = auth.uid()
  and status = 'draft'
  and score = 0
  ...
  and exists (
    ...
    where ca.id = class_assignment_id
      and ca.published_at is not null
      and cm.student_id = auth.uid()
  )
);
create policy "students update own ungraded submissions" on public.submissions for update to authenticated
...
create policy "teachers grade class submissions" on public.submissions for update to authenticated
using (
  exists (
    select 1 from public.class_assignments ca join public.exercises e on e.id = ca.exercise_id
    where ca.id = class_assignment_id and e.teacher_id = auth.uid()
